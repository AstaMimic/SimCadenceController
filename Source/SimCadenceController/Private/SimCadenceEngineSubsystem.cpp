...
void USimCadenceEngineSubsystem::Initialize(FSubsystemCollectionBase& Collection)
{
    ...
    WorldInitHandle = FWorldDelegates::OnPostWorldInitialization.AddUObject(this, &USimCadenceEngineSubsystem::OnWorldInit);
    WorldCleanupHandle = FWorldDelegates::OnWorldCleanup.AddUObject(this, &USimCadenceEngineSubsystem::OnWorldDestroyed);

    switch (S->Mode)
    {
        case ESimCadenceMode::Realtime:          ApplyRealtimeMode(); break;
        case ESimCadenceMode::TrainingRendered:  ApplyTrainingMode(false); break;
        case ESimCadenceMode::TrainingHeadless:  ApplyTrainingMode(true); break;
    }
}
...
{
    if (WorldInitHandle.IsValid())  { FWorldDelegates::OnPostWorldInitialization.Remove(WorldInitHandle); }
...
void USimCadenceEngineSubsystem::ApplyTrainingMode(bool bHeadless)
{
    ...
    {
        if (S->bUncapInTraining) { PresentInterval = 0.0; }
        else { PresentInterval = 1.0 / FMath::Max(1.f, S->TrainingRenderCapHz); }
    }
...
ASimCadencePhysicsBridge* USimCadenceEngineSubsystem::GetOrSpawnPhysicsBridge(UWorld* World)
{
    ...
    ASimCadencePhysicsBridge* Bridge = World->SpawnActor<ASimCadencePhysicsBridge>(ASimCadencePhysicsBridge::StaticClass());
